...
// Industry-grade Express + PostgreSQL + JWT backend
// Vercel-ready: reads DATABASE_URL (Supabase connection pooling URL)
// ─────────────────────────────────────────────────────────────────
...
let SECRET = process.env.JWT_SECRET; // re-assigned below if missing
...
if (!SECRET || (IS_PROD && SECRET.length < 32)) {
  logger.error('[server.js] ❌ JWT_SECRET is missing or too short. Using a fallback — login will be broken!');
  // Do NOT crash with process.exit() on Vercel — the function would return a blank 500.
  SECRET = 'INSECURE_FALLBACK_SECRET_PLEASE_CHANGE_IMMEDIATELY_1234567890';
}
...
//   • any *.vercel.app subdomain (Vercel deployments)
...
app.use(cors({
  origin: (origin, cb) => {
    ...
    if (!origin || (origin === 'null' && !IS_PROD)) return cb(null, true); // (1g fix) Guard null origin
...
function handleDbError(res, err, context) {
  ...
  if (err.code === '42P01' || err.code === '3D000')
    return fail(res, 'Database tables not found. Run schema.sql against your Supabase PostgreSQL first.', 503);
  if (err.code === 'ECONNREFUSED' || err.code === '28P01' || err.code === '28000')
    return fail(res, 'Cannot connect to PostgreSQL. Check DATABASE_URL in Environment Variables.', 503);
...
app.get('/api/ping', async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: 'OK — PostgreSQL connected.' });
  } catch (err) {
    return res.status(503).json({ success: false, message: 'PostgreSQL not connected: ' + err.message });
  }
});
  ...
  if (cluster.isPrimary && IS_PROD) {
    ...
    const gracefulExit = () => {
      ...
      server.close(async () => {
        ...
        try {
          await pool.end();
          logger.info(`[Worker ${process.pid}] Closed PostgreSQL pool.`);
        } catch (err) {
          logger.error(`[Worker ${process.pid}] Error closing PostgreSQL pool: ${err.message}`);
        }
        process.exit(0);
      });
