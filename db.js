...
// Reads DATABASE_URL (Supabase connection pooling URL) for Vercel/production.
// Falls back to PGHOST / PGUSER / PGPASSWORD / PGDATABASE for local development.
import pg from 'pg';
...
if (!process.env.DATABASE_URL && (!DB_HOST || !DB_USER || !DB_NAME)) {
  ...
  // Removed process.exit(1) to prevent Vercel 500 crashes; let it fail gracefully on connection.
}
